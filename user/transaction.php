<?php
// user/transaction.php - Complete transaction page with payment code display
...
// Get payment code from URL or database
$generated_code = '';
$show_payment_code = false;

// Check if code was passed in URL
if (isset($_GET['code']) && strlen($_GET['code']) == 5 && ctype_digit($_GET['code'])) {
    $generated_code = $_GET['code'];
    $show_payment_code = true;
} else {
    // Check if there's an existing pending code for this user
    $code_check = $conn->query("
        SELECT pc.code FROM payment_codes pc
    ...
    if ($code_check && $code_check->num_rows > 0) {
        $generated_code = $code_check->fetch_assoc()['code'];
        $show_payment_code = true;
    }
}
            ...
            try {
                ...
                if ($is_buyer) {
                    ...
                    if ($check['escrow_held'] >= $required) {
                        ...
                        $payment_success = "Payment confirmed! Both deposits are complete. Proceed to legal process.";
                    } else {
                        $payment_success = "Seller deposit confirmed! Waiting for buyer payment.";
                    }
                }
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .step-circle { width: 56px; height: 56px; background: #e2e8f0; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 12px; font-size: 24px; transition: all 0.3s; }
        .step.completed .step-circle { background: #10b981; color: white; }
        .step.active .step-circle { background: #667eea; color: white; transform: scale(1.1); box-shadow: 0 8px 20px rgba(102,126,234,0.3); }
        ...
        .error-message { background: #fee2e2; color: #dc2626; padding: 14px 18px; border-radius: 16px; margin-bottom: 20px; display: flex; align-items: center; gap: 12px; }
        .success-message { background: #d1fae5; color: #059669; padding: 14px 18px; border-radius: 16px; margin-bottom: 20px; display: flex; align-items: center; gap: 12px; }
        ...
        .pin-input { padding: 14px; font-size: 20px; text-align: center; letter-spacing: 8px; border: 2px solid #e2e8f0; border-radius: 16px; font-family: monospace; }
        ...
        <div class="status-timeline">
            ...
            <div class="timeline-steps">
                ...
                <div class="step <?php echo $currentStep >= 1 ? 'completed' : ($bothDepositsPaid ? 'active' : ''); ?>">
                    ...
                    <div class="step-desc"><?php echo $buyerPaid > 0 ? 'Buyer ✓' : 'Buyer ⏳'; ?> | <?php echo $sellerPaid > 0 ? 'Seller ✓' : 'Seller ⏳'; ?></div>
                </div>
                <div class="step <?php echo $currentStep >= 2 ? 'completed' : ($currentStep == 1 ? 'active' : ''); ?>">
                    ...
                    <div class="step-desc"><?php echo $legal_status['buyer_confirmed'] ? 'Buyer ✓' : 'Buyer ⏳'; ?> | <?php echo $legal_status['seller_confirmed'] ? 'Seller ✓' : 'Seller ⏳'; ?></div>
                </div>
                <div class="step <?php echo $currentStep >= 3 ? 'completed' : ($currentStep == 2 ? 'active' : ''); ?>">
                    ...
                    <div class="step-desc"><?php echo $delivery_status['buyer_confirmed'] ? 'Buyer ✓' : 'Buyer ⏳'; ?> | <?php echo $delivery_status['seller_confirmed'] ? 'Seller ✓' : 'Seller ⏳'; ?></div>
                </div>
        ...
        <?php if ($transaction['status'] != 'completed' && $transaction['status'] != 'cancelled'): ?>
                ...
                <div class="card">
                    ...
                    <?php if ($show_payment_code && $generated_code): ?>
                        <div class="payment-code-box">
                            ...
                            <p class="step-desc" style="margin-top: 15px;">
                                <i class="fas fa-hourglass-half"></i> Code expires in 10 minutes
                            </p>
                        </div>
                        ...
                        <div class="confirmation-section">
                            ...
                            <form method="POST">
                                ...
                                <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap; margin-top: 16px;">
                                    <input type="text" name="payment_code_confirm" placeholder="Enter 5-digit code" maxlength="5" pattern="\d{5}" required class="pin-input" style="flex: 1;">
                    ...
                    <?php else: ?>
                        <div class="confirmation-section" style="text-align: center;">
                            <p>Click the button below to generate your payment code.</p>
                            <a href="transaction.php?id=<?php echo $transaction_id; ?>&code=generate" class="btn btn-primary" style="margin-top: 16px;">
            ...
            <?php if ($is_seller && $sellerRemaining > 0): ?>
                <div class="card">
                    ...
                    <div class="payment-code-box" style="background: linear-gradient(135deg, #10b981, #059669);">
                        <h3>📱 Your Telebirr Payment Code</h3>
                        <div class="payment-code" id="sellerPaymentCode" style="background: #064e3b; color: white;">GENERATE CODE</div>
                        <a href="transaction.php?id=<?php echo $transaction_id; ?>&code=generate" class="btn btn-outline">Generate Payment Code</a>
                    </div>
                </div>
            <?php endif; ?>
            ...
            <?php if ($delivery_status['both_confirmed'] && $transaction['status'] == 'deposits_complete' && $is_buyer): ?>
                <div class="card">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="badge badge-success">Confirmed</span></td>                        </tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
            </div>
