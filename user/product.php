...
// FIXED: Jobs show only service fee, no deposit or remaining balance
...
// Determine if this is a job listing
$is_job = ($listing['type'] == 'job');
...
// For jobs, only service fee (commission) is required
if ($is_job) {
    $totalPayment = $commissionAmount;      // Only pay service fee
    $remainingAmount = 0;                   // No remaining balance
    $displayDepositAmount = 0;              // No deposit for display
    $depositPercent = 0;                    // No deposit percentage
} else {
    $totalPayment = $depositAmount + $commissionAmount;
    $remainingAmount = $listing['price'] - $depositAmount;
    $displayDepositAmount = $depositAmount;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['purchase']) && !$is_seller && $listing['type'] != 'rental') {
    ...
    if (!$is_available_for_booking && !$is_reserved_by_me) {
        ...
        // For jobs, deposit_amount = 0, remaining_balance = 0
        $insertDepositAmount = $is_job ? 0 : $depositAmount;
        $insertRemainingAmount = $is_job ? 0 : $remainingAmount;
        ...
        ");
        $stmt->bind_param("iiiiddd", $listing_id, $buyer_id, $listing['seller_id'], $listing['price'], $insertDepositAmount, $commissionAmount, $insertRemainingAmount);
    ...
    <div class="product-container">
        ...
        <div class="sidebar">
            ...
            <div class="payment-breakdown">
                ...
                <div class="breakdown-item">
                    <span><?php echo $is_job ? 'Service Fee' : 'Service Fee'; ?> (<?php echo $commissionPercent; ?>%)</span>
                ...
                <?php if (!$is_job && $listing['type'] == 'rental'): ?>
                <div class="breakdown-item">
            ...
            <?php if ($is_seller): ?>
                <?php if ($listing['type'] == 'job'): ?>
                    <!-- JOB SELLER VIEW - Show service fee info only -->
                    <div class="payment-breakdown" style="border: 1px solid #dbeafe; background: #eef2ff;">
                        <div class="breakdown-item">
                        ...
                        <div class="breakdown-item total">
                            ...
                            <span><?php echo formatMoney($listing['price'] - $commissionAmount); ?></span>
                        </div>
                        <div class="info-text" style="margin-top: 12px; text-align: center; background: #dbeafe; padding: 8px; border-radius: 12px;">
                            <i class="fas fa-info-circle"></i> Applicants pay the service fee. You receive the full salary after successful completion.
                        </div>
                    </div>
                <?php elseif ($seller_payment && $seller_payment['has_deposit_payment']): ?>
                    <!-- PRODUCT/RENTAL SELLER VIEW - Show deposit and remaining balance -->
                ...
                <div class="alert alert-info">
                    ...
                    <div>This is your <?php echo $listing['type']; ?> listing. <?php echo ($listing['type'] == 'job') ? 'Manage applications from your dashboard.' : 'View bookings in "My Renters".'; ?></div>
                </div>
                ...
                <?php if ($is_available_for_booking): ?>
                    <form method="POST">
                        ...
                        <button type="submit" class="btn-purchase">
                            <i class="fas fa-shopping-cart"></i> <?php echo $is_job ? 'Apply Now' : 'Purchase Now'; ?>
                        </button>
                    </form>
                    <p style="font-size: 11px; color: var(--gray); text-align: center; margin-top: 12px;">
                        <i class="fas fa-shield-alt"></i> <?php echo $is_job ? 'Pay service fee to submit application' : 'Pay deposit to secure this item'; ?>
                    </p>
                <?php elseif ($is_reserved_by_me): ?>
                    <?php $txn_id = getExistingTransactionId($conn, $listing_id, $user_id); ?>
                    <a href="pay_rent.php?transaction_id=<?php echo $txn_id; ?>" class="btn-purchase">
